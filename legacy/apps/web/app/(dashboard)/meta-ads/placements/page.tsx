...
import { metaPlacements } from "@/lib/mock-data/meta-ads";
...
export default function MetaPlacementsPage() {
  return (
    <div>
      <TopBar subtitle="Results by placement across Facebook and Instagram surfaces." />
            ...
            <tbody>
              {metaPlacements.map((p) => (
                <tr key={p.placement} className="border-b border-slate-50 last:border-0">
                  ...
                  <td className="py-2.5 text-neutral">{p.results}</td>
                  <td className="py-2.5 font-medium text-success">{p.roas}</td>
                </tr>
              ))}
            </tbody>
          </table>
